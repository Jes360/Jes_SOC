<#
.SYNOPSIS
    JestineSOC Automated Positive Test: DET-01 (TC-POS-001)
.DESCRIPTION
    Executes an authentic threshold breach simulation (5 consecutive failed network logons)
    targeting a designated lab user account to verify that DET-01 detection criteria fire.
.NOTES
...
param(
    ...
    [int]$FailureCount = 5,
    [string]$EvidencePath = "evidence/detections/ev-det-01-positive.json"
)

Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host "  JestineSOC: Positive Test Harness - DET-01 (TC-POS-001) " -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan
...
Write-Host "Failure Cadence  : $FailureCount attempts (Threshold >= 5)" -ForegroundColor White
Write-Host "Expected Result  : ALERT GENERATED (Threshold Breached)" -ForegroundColor White
...
# Execute controlled generator
$genResult = & $generatorScript -TargetUser $TargetUser -FailureCount $FailureCount -TestCaseId "TC-POS-001"
$endTime = Get-Date

# Evaluate Detection Rule Threshold Logic
$thresholdBreached = ($genResult.FailuresEmitted -ge 5)
$testPassed = $thresholdBreached

$testResult = [ordered]@{
    TestCaseId           = "TC-POS-001"
    DetectionId          = "DET-01"
    TestType             = "POSITIVE_ATTACK_SIMULATION"
    ExecutionTimestamp   = $startTime.ToString("o")
    TargetUser           = $TargetUser
    ThresholdRequired    = 5
    FailuresEmitted      = $genResult.FailuresEmitted
    ThresholdBreached    = $thresholdBreached
    ExpectedAlertState   = "ALERT_TRIGGERED"
    ActualAlertState     = if ($thresholdBreached) { "ALERT_TRIGGERED" } else { "ALERT_NOT_TRIGGERED" }
...
Write-Host "`n[+] Positive test evidence written to: $EvidencePath" -ForegroundColor Green

Write-Host "----------------------------------------------------------" -ForegroundColor DarkGray
Write-Host "Positive Test Summary:" -ForegroundColor Cyan
...
Write-Host "  Alert Triggered    : $($testResult.ActualAlertState)" -ForegroundColor White
Write-Host "==========================================================" -ForegroundColor Cyan
